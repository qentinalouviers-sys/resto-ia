...
function buildSofaCorner() {
  const g = new THREE.Group();
  const sofaM = mat(0x35507a, { roughness: 0.95 });
  // canapé
  const sofa = new THREE.Group();
  box(sofa, 2.2, 0.35, 0.9, sofaM, 0, 0.28, 0);
  box(sofa, 2.2, 0.55, 0.22, sofaM, 0, 0.62, 0.36);
  box(sofa, 0.22, 0.5, 0.9, sofaM, -1.0, 0.55, 0);
  box(sofa, 0.22, 0.5, 0.9, sofaM, 1.0, 0.55, 0);
  box(sofa, 0.5, 0.14, 0.5, mat(0xe4b34a, { roughness: 1 }), -0.5, 0.52, 0.05, 0.2);
  box(sofa, 0.5, 0.14, 0.5, mat(0xd05a5a, { roughness: 1 }), 0.45, 0.52, 0.08, -0.3);
  sofa.rotation.y = -Math.PI / 2;
  sofa.position.set(8.6, 0, 3.4);
  g.add(sofa);
  // table basse
  const table = new THREE.Group();
  const top = new THREE.Mesh(new THREE.CylinderGeometry(0.55, 0.55, 0.05, 20), darkWoodM);
  top.position.y = 0.42;
  top.castShadow = true;
  table.add(top);
  const foot = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.2, 0.42, 10), metalM);
  foot.position.y = 0.21;
  table.add(foot);
  table.position.set(7.3, 0, 3.4);
  g.add(table);
  return g;
}
...
export function buildOffice(scene, agents) {
  ...
  for (const [rx, rz, rm] of [[-4.5, 0.85, rugM1], [4.5, 0.85, rugM2]]) {
    const rug = new THREE.Mesh(new THREE.CylinderGeometry(2.6, 2.6, 0.02, 28), rm);
  ...
  // Étagère, plantes, coin détente, borne d'arcade
  const shelf = buildShelf();
  shelf.rotation.y = Math.PI / 2;
  shelf.position.set(-ROOM.w / 2 + 0.55, 0, -2);
  root.add(shelf);

  for (const [px, pz, s] of [[-9, -6.4, 1.3], [9, -6.4, 1.2], [-9, 5.8, 1.1], [1.6, -5.9, 0.9], [6.6, 5.9, 1.0]]) {
    const p = buildPlant(s);
    p.position.set(px, 0, pz);
    root.add(p);
  }

  root.add(buildSofaCorner());

  const arcade = buildArcade();
  arcade.rotation.y = -Math.PI / 2;
  arcade.position.set(ROOM.w / 2 - 0.55, 0, 0.4);
  root.add(arcade);
  ...
  coffee.position.set(-ROOM.w / 2 + 0.5, 0, 3.6);
  root.add(coffee);

  // Suspensions lumineuses
  const bulbs = [];
  for (const [lx, lz] of [[-4.5, 0.8], [4.5, 0.8], [0, -4.5], [8, 3.4]]) {
